...
public class WailaConfigScreen extends ConfigScreen {
    ...
    private class PlacementScreen extends YesIAmSureTheClientInstanceIsPresentByTheTimeIUseItScreen {
        ...
        boolean drag;
        Vector3d dragPos = new Vector3d(0, 0, 0);
        Vector3d dragStart = new Vector3d(0, 0, 0);
        Vector3d dragNow = new Vector3d(0, 0, 0);
        ...
        protected void init() {
            ...
            addRenderableWidget(done = new Button(0, 0, 102, 20, CommonComponents.GUI_DONE, (b) -> minecraft.setScreen(WailaConfigScreen.this)));
        ...
        public void tick() {
            ...
            xAnchor.x = optX;
            xAnchor.y = y + (22 * i++);
            yAnchor.x = xAnchor.x + xAnchor.getWidth() + 2;
            yAnchor.y = xAnchor.y;

            xAlign.x = optX;
            xAlign.y = y + (22 * i++);
            yAlign.x = xAlign.x + xAlign.getWidth() + 2;
            yAlign.y = xAlign.y;

            xPos.x = optX;
            xPos.y = y + (22 * i++);
            yPos.x = xPos.x + xPos.getWidth() + 2;
            yPos.y = xPos.y;
        ...
        public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
            ...
            // @formatter:off
            GuiComponent.drawString(matrices, minecraft.font, TEXT_ANCHOR, x, y + (22 * i++), 0xFFFFFFFF);
            GuiComponent.drawString(matrices, minecraft.font, TEXT_ALIGN , x, y + (22 * i++), 0xFFFFFFFF);
            GuiComponent.drawString(matrices, minecraft.font, TEXT_OFFSET, x, y + (22 * i++), 0xFFFFFFFF);
            GuiComponent.drawString(matrices, minecraft.font, TEXT_SCALE , x, y + (22 * i++), 0xFFFFFFFF);
            // @formatter:on

            TooltipRenderer.render(matrices, delta);
        }
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            if (rect.contains(mouseX, mouseY)) {
                drag = true;
                dragPos.set(xPosValue.getValue(), yPosValue.getValue(), 0);
